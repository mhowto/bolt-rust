...
pub struct Bucket<'a> {
    ...
    buckets: HashMap<&'static str, Arc<Bucket<'a>>>, // subbucket cache
    // page: &'b Page, // inline page reference
    root_node: Arc<Node<'a>>, // materialized node for the root page.
    nodes: HashMap<pgid_t, Arc<Node<'a>>>, // node cache
...
impl <'a> Bucket<'a> {
    pub fn new(b: Box<_Bucket>, tx: Arc<Tx>, root: Arc<Node<'a>>) -> Bucket<'a> {
        Bucket{
            bucket: b,
            tx: tx,
            buckets: HashMap::new(),
